...
Example:
  dane = array('TA-150-6-7|nazwa|[wartosc]')
  values = array('nazwa'=>wartosc);
...
*/
 ...
 class FormClass {
   ...
   var $m_combo = array();
   var $m_comboId = array();
   var $m_comboValue = array();
      
   function __construct() {

   }

   function SetCombo($name, $combo, $comboId, $comboValue=false) {
   
      if (!$comboValue) $comboValue = $comboId;
      $this->m_combo[$name] = $combo;
      $this->m_comboId[$name] = $comboId;
      $this->m_comboValue[$name] = $comboValue;
   }
   
   function MakeForm($dane, $values, $errors=array()) {
   ...
   // private
   function GetCode($ntyp, $column, $insert) {
        $tmp = explode('-', $ntyp);
        $ile = count($tmp) ;
        $typ = $tmp[0];
        $kod = false;
        ...
        switch ($typ) {
            case 'T':
                ...
                if (isset($this->m_errors[$column]))  { $fclass = ' class="form_error" ';}
                else {$fclass = '';}
