<?php
  ...
  class SitesClass {
    ...
    function AddSite($a_values,  $a_payment,  $sub_id) {
      ...
      if ($id > 0 and $pid) {
        if ($this->AddConnection($id, $sub_id) !== false) {
           ...
           $this->m_DB->UpdateQuery($this->m_table, array('last_payment_id'=>$pid), $id);   
    ...
    function UpdateSite( $a_values, $id) {
    ...
    function AddPayment($a_payment) {
      ...
      if ($id) {
        $this->m_DB->UpdateQuery($this->m_table, array('last_payment_id'=>$id), $a_payment['site_id']);
      }
      return $id;
    }

    function UpdatePayment($a_payment, $id) {
    ...
    function AddConnection($id, $sub_id) {
      ...
      if (!$this->IsConnection($id, $sub_id)) {
         $ret = $this->m_DB->InsertQuery($this->m_connection, array('site_id'=>$id, 'sub_id'=>$sub_id));
         if ($ret===false) {
    ...
    function DeleteConnection($id, $sub_id) {
      $ret = false;
    ...
    function GetListingByID_CodeEmail($id_code,$email) {
    ...
    function GetSites($categ_id, $page, $sort_by) {
    ...
    function Latest($link_type=LT_FEAT,$limit_num=4) {
    ...
    function MostVisited($link_type=LT_FEAT,$limit_num=5) {
    ...
    function CountSitesInCateg($categ_id) {
    ...
    function GetSitesByType($types, $page=1) {
      $begin = ($page-1)*30;
    ...
    function SearchSites($params, $page=1) {
    ...
    function UserSearch($search, $page=1) {
      $begin = ($page-1)*30;
    ...
    function UserPhoneSearch($search) {
    ...
    function SearchApprove($search, $page=1) {
      $begin = ($page-1)*30;
    ...
    function SearchActive($search, $page=1) {
      $begin = ($page-1)*30;
    ...
    function SearchInactive($search, $page=1) {
      $begin = ($page-1)*30;
    ...
    function UserSearchCategs($search) {
      $begin = 0; // ($page-1)*30;
    ...
    function CheckRecip($url, $url_find) {
    ...
    // filters
    function CheckWords($text0, $text1, $text2, $text3, $text4, $text5, $banned_words) {
      if (!is_array($banned_words)) return false;
      $tmp = '';
    ...
    function CheckIp($ip, $banned_ips) {
	...
	function UpdateVisit($id,$ip,$date) {
	  $res = true;
	...
	function RecordVisit($id,$ip,$date) {
	  $res = true;
    ...
    function HrsDiff($earlierdate,$laterdate) {
    ...
    function GetTitle($data) {
      $dirty = array("content", "Content", "CONTENT", "\r", "\n", "=","\"","/", ">" ,"<");
    ...
    function Between2($text, $sFind, $eFind) {
       $extract = false;   
