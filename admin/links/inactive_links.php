<?php
  ...
  if ($g_user->Level() == AL_ADMIN) {
      ...
      $template = ADMIN_TEMPLATE_DIR.'links_inactive_links.html';
      ...
      $query = "SELECT s.*, p.paid, p.id AS payment_id, p.error, p.is_error, p.amount, p.email AS pemail, p.payment_period, p.business, p.payment_date FROM $g_site->m_table s ".
        " LEFT JOIN $g_site->m_payment p ON s.last_payment_id=p.id  
        WHERE status=".SITE_BANNED.$order_by.$limit;
      ...
      foreach ($table as $site ){
        ...
        foreach ($categories as $category) {
          ...
          // }
          $categories_in .= ' - <a href="'.$categ_address.'" target="_blank">'.$category['name'].'</a> <br>';
      ...
      $site_tpl['{lang inactive_title}'] = LANG_INACTIVE_TITLE;            
