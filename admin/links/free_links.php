<?php
  ...
  if ($g_user->Level() == AL_ADMIN) {
      ...
      $query = "SELECT COUNT(s.id) as hmany FROM $g_site->m_table s
         WHERE status=".SITE_WAITING. ' AND s.paid_link=0';
      ...
      $query = "SELECT s.* FROM $g_site->m_table s ".
        " WHERE status=".SITE_WAITING.' AND s.paid_link=0 '.$order_by.$limit;
      ...
      $site_tpl['{lang approve_title}'] = LANG_FREE_LINKS;         
