...
if ($g_user->Level() != AL_ADMIN) {
    if ($site['status'] != SITE_VIEW) {header("location: 404.php");}
}
...
if ($site['link_type'] == LT_ADD3) {$hm_deep_links = 3;} else if ($site['link_type'] == LT_ADD5) {$hm_deep_links = 5;} else {
    $deep_links                      = ' ';
...
$tmp_tpl->IfRegion(['isadmin' => $bIsAdmin, 'show_pagerank' => COLLECT_PAGE_RANK, 'logo_display' => $logo_disp, 'video_display' => $video_disp]);
