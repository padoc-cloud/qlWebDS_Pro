...
if ($g_user->Level() == AL_ADMIN) {
    ...
    if (isset($_POST['make_backup'])) {
        ...
        if (! $gp) {
            ...
            $db_tables = ['captcha', 'categories', 'connection', 'params', 'payment', 'session', 'sites', 'users', 'visits'];
            ...
            foreach ($db_tables as $db_table) {
                ...
                foreach ($table as $row) {
                    ...
                    $row = array_map(function ($value) use ($DB) {
                        return mysqli_real_escape_string($DB->m_conn, $value);
                    }, $row);
    ...
    $backupDir = MAIN_CATALOG . 'backup/';
    $arDir     = [];
    $arlDir    = [];

    if (is_dir($backupDir)) {
        $entries = scandir($backupDir); // Use scandir instead of dir()
        foreach ($entries as $entry) {
            if ($entry !== '.' && $entry !== '..') {
                $fullPath = $backupDir . DIRECTORY_SEPARATOR . $entry;

                // DB files (ending with .sql.gz)
                if (! is_dir($fullPath) && substr($entry, -6) === 'sql.gz') {
                    $arDir[] = $entry;
                }

                // Logos files (ending with logos.zip)
                if (! is_dir($fullPath) && substr($entry, -9) === 'logos.zip') {
                    $arlDir[] = $entry;
                }
            }
        }
