...
if ($g_user->Level() == AL_ADMIN) {
    ...
    if (isset($_GET['delete_id'])) {
        ...
        $g_site->SendEmail($g_params->GetParams('site_not_approved_email'), $site);
    ...
    $query = "SELECT COUNT(id) as hmany FROM $g_site->m_table s WHERE status=" . SITE_WAITING;
    $row   = $DB->GetRow($query);
    ...
    foreach ($table as $site) {
        ...
        $logo_filename = $site['id'] . LOGO_NAME_SUFFIX;
        $sites[$i]['{logo}'] = SITE_ADDRESS . LOGO_DIR . "/" . $logo_filename;
