...
class UploadClass
{
    ...
    public $max_photo_size = MAX_LOGO_SIZE;
    public $m_dir;
    public $m_name;
    ...
    public function UploadClass($dir, $name, $max = 1, $aFilter = ['jpg', 'png', 'gif', 'bmp', 'jpeg'])
    {
    ...
    */
    ...
    public function ResizeImg($input, $output, $max_width, $max_height)
    {
        ...
        if ($width > 0 and $height > 0) {
            ...
            switch ($what['mime']) {
                case 'image/png':$source = imagecreatefrompng($input) or die($gd2er = true);
                    $t                       = 1;
                    break;
                case 'image/jpeg':$source = imagecreatefromjpeg($input) or die($gd2er = true);
                    $t                        = 2;
                    break;
                case 'image/gif':$source = imagecreatefromgif($input) or die($gd2er = true);
                    $t                       = 3;
                    break;
                case 'image/bmp':$etext = 'BMP images not supported';
                    break;

                default:$etext .= "Unknown image type. <br> ";
                    break;
            }
            ...
            if (! $gd2er) {
                ...
                if ($ret) {
                    switch ($t) {
                        case 1:$ret = imagepng($new_image, $output);
                            break;
                        case 2:$ret = imagejpeg($new_image, $output, 90);
                            break;
                        case 3:$ret = imagegif($new_image, $output);
                            break;
                    }
                }
