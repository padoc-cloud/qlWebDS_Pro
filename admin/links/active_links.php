<?php
  ...
  if ($g_user->Level() == AL_ADMIN) {
      ...
      $template = ADMIN_TEMPLATE_DIR.'links_active_links.html';
      ...
      $query = "SELECT s.*, p.paid, p.id AS payment_id, p.error, p.is_error, p.amount, p.email AS pemail, p.currency, p.payment_period, p.business, p.payment_date 
        ...
        WHERE status=".SITE_VIEW.$order_by.$limit;
      ...
      $query = "SELECT ct.*, cn.*  FROM $g_categ->m_connection AS cn 
        ...
        WHERE cn.site_id=st.id AND st.status=".SITE_VIEW;
      ...
      foreach ($table as $site ){
        ...
        // inactivate expired listings
        if (isset($_POST['submit_inact']) and ($site['exp_date']<>'0000-00-00 00:00:00') and 
          ($site['exp_date'] < date(DATETIME_FORMAT))) {
          	$values_in['status'] = SITE_BANNED;
          	if ($g_site->UpdateSite($values_in,$site['id'])) {
				$inact++;          		 
          	}
        } else {
	        ...
	        // site info address
	        if (MOD_REWRITE) {
	          $more_address = CATALOG_ADDRESS.str_replace(',',WORD_SEPARATOR,$site['mod_rewrite']).','.MOD_SITE.'-'.$site['id'].PAGE_EXTENSION;
	        } else {
      ...
      $site_tpl['{lang submit_inact}'] = LANG_INACT_EXP;
	  ...
	  if (isset($_POST['submit_inact'])) {
	      $site_tpl['{inact style}'] = '';
	      $site_tpl['{inact message}'] = '<div class="info">'.LANG_INACT_EXP_RESULT.': '.$inact.'</div>';
	  } else {
	      $site_tpl['{inact style}'] = 'style="display: none;"';
	      $site_tpl['{inact message}'] = '';
	  }
