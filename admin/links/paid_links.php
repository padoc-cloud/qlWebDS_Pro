<?php
  ...
  if ($g_user->Level() == AL_ADMIN) {
      ...
      $query = "SELECT COUNT(s.id) as hmany FROM $g_site->m_table s, $g_site->m_payment p
         WHERE status=".SITE_WAITING. ' AND s.last_payment_id=p.id AND s.last_payment_id=p.id p.paid='.PM_PAID;

      $query = "SELECT COUNT(s.id) as hmany FROM $g_site->m_table s
          LEFT JOIN $g_site->m_payment p ON s.last_payment_id=p.id 
         WHERE status=".SITE_WAITING." AND p.paid=".PM_PAID ;
      ...
      $query = "SELECT s.*, p.paid, p.id AS payment_id, p.error, p.is_error, p.amount, p.currency, p.email AS pemail, p.payment_period, p.business, p.payment_date FROM $g_site->m_table s ".
        " LEFT JOIN $g_site->m_payment p ON s.last_payment_id=p.id 
        WHERE status=".SITE_WAITING." AND p.paid=".PM_PAID.$order_by.$limit;
      ...
      $site_tpl['{lang approve_title}'] = LANG_PAID_LINKS;         
